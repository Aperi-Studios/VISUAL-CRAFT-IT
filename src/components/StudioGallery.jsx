  ...
  return (
    <section ref={containerRef} className="relative h-screen bg-black overflow-hidden">
      <div className="sticky top-0 h-screen flex items-center overflow-hidden">
        ...
        <div ref={wrapperRef} className="flex gap-8 px-[20vw]">
          {IMAGES.map((img, i) => (
            <div 
              key={i} 
              className="relative flex-shrink-0 w-[80vw] md:w-[45vw] aspect-[16/9] overflow-hidden rounded-3xl border border-white/5 group shadow-2xl"
            >
              ...
              <div className="absolute bottom-8 left-8 flex items-center gap-4">
                ...
                <span className="text-[10px] font-mono text-white/40 tracking-[0.3em] uppercase">CAM_LOG // 00{i+1}</span>
              </div>
            </div>
          ))}
          
          <div className="flex-shrink-0 w-[80vw] md:w-[35vw] aspect-[16/9] bg-white/[0.01] border border-white/5 rounded-3xl p-12 flex flex-col justify-center backdrop-blur-3xl">
             <span className="text-[10px] font-black uppercase tracking-[0.4em] text-zinc-600 mb-8 block">Hardware Authority —</span>
             <p className="text-white text-3xl md:text-4xl font-black uppercase tracking-tighter mb-12 leading-[0.9]">
               Equipped for <br /> global scale.
             </p>
             <div className="space-y-8">
                <div className="flex justify-between border-b border-white/5 pb-2 group cursor-default">
                   <span className="text-[10px] font-black uppercase tracking-widest text-zinc-500 group-hover:text-white transition-colors">Render Farm</span>
                   <span className="text-[11px] font-bold text-white uppercase">12.4 Petaflops</span>
                </div>
                <div className="flex justify-between border-b border-white/5 pb-2 group cursor-default">
                   <span className="text-[10px] font-black uppercase tracking-widest text-zinc-500 group-hover:text-white transition-colors">Pipeline</span>
                   <span className="text-[11px] font-bold text-white uppercase">USD-Native</span>
                </div>
                <div className="flex justify-between border-b border-white/5 pb-2 group cursor-default">
                   <span className="text-[10px] font-black uppercase tracking-widest text-zinc-500 group-hover:text-white transition-colors">Security</span>
